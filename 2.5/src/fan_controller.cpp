...
public:
    ...
    static constexpr uint64_t TICK_US          = 100000ULL;
...
public:
    ...
    static void logState(FanState s, uint64_t nextEventUs) {
        ...
        const uint64_t leftUs = (nextEventUs > nowUs) ? (nextEventUs - nowUs) : 0ULL;
...
void setup() {
    ...
    Logger::log("Watchdog configured");
    ...
    ESP_ERROR_CHECK(esp_timer_create(&args, &gTimer));
    ESP_ERROR_CHECK(esp_timer_start_periodic(gTimer, Config::TICK_US));
